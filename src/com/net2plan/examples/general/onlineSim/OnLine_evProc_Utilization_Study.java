...
import com.net2plan.libraries.FlexGridUtilsJavi.ModulationFormat;
...
//		double[] u_e = initialNetPlan.getLinkCapacityVector();
		...
		if (distanceAdaptive.getBoolean ())
		{
			...
			modulationFormatPerPath = FlexGridUtilsJavi.computeModulationFormatPerPath(cpl, linkLengthInKmMap, candidateModulationFormats);
			...
			for (ModulationFormat modulationFormat : candidateModulationFormats)
			{
				for (int serviceId = 0; serviceId < numServices; serviceId++)
					numSlotsPerService[serviceId] = FlexGridUtilsJavi.computeNumberOfSlots(bandwidthInGbpsPerServiceArray[serviceId], slotGranularityInGHz, guardBandInGHz.getDouble (), modulationFormat);
		...
		{		
			...
			numSlotsPerModulationPerService = new LinkedHashMap<ModulationFormat, int[]>();
			for (int serviceId = 0; serviceId < numServices; serviceId++)
				numSlotsPerService[serviceId] = FlexGridUtilsJavi.computeNumberOfSlots(bandwidthInGbpsPerServiceArray[serviceId], slotGranularityInGHz, guardBandInGHz.getDouble (), routeModulationFormat);
		...
		statsCalculator.setStatisticsCalculator(initialNetPlan,trafficLayer,numSlotsPerService,totalAvailableSlotsPerFiber,this.totalAvailableSlotsPerFiber);
	...
	private Quadruple<List<Link>, ModulationFormat, Integer, Integer> findPotentialAllocationForConnection(NetPlan currentNetPlan, Demand thisDemand , double lineRateGbps , int serviceId)
	{
		...
		else if (this.rsaAlgorithmType.getString ().equalsIgnoreCase("fFFragmentationAware"))
		{
			...
			for (List<Link> seqFibers : paths)
			{
				...
				final TreeSet<Integer> pathSlotOccupancy = FlexGridUtilsJavi.computePathSlotOccupancy(seqFibers, fiberSlotOccupancyMap_fs, totalAvailableSlotsPerFiber);
...
//					System.out.println("---- Algebraic Connectivity is calculated ----");
		...
		{			
			...
			{
				final TreeSet<Integer> pathSlotOccupancy = FlexGridUtilsJavi.computePathSlotOccupancy(bestSeqFibers, fiberSlotOccupancyMap_fs, totalAvailableSlotsPerFiber);
				final List<Pair<Integer, Integer>> candidateVoids = FlexGridUtils.computeAvailableSpectrumVoids(pathSlotOccupancy, totalAvailableSlotsPerFiber);
