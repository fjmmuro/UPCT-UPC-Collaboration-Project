...
public class Online_evGen_wdm extends IEventGenerator
{
	...
	private InputParameter is_flexgridType = new InputParameter("is_flexgridType", (Boolean) true, "Acitve value if the simulation is over elastic networks");
	...
	public void initialize(NetPlan initialNetPlan, Map<String, String> algorithmParameters, Map<String, String> simulationParameters, Map<String, String> net2planParameters)
	{
		...
		this.isFlexgrid = is_flexgridType.getBoolean();
		...
		/* Initialize Flexgrid Parameters if it is selected */
		if (isFlexgrid)
		{
			if (fg_loadFactor.getDouble() <= 0) throw new Net2PlanException("Network load factor must be greater than zero");		
			this.bandwidthInGbpsPerServiceArray = StringUtils.toDoubleArray(StringUtils.split(fg_bandwidthInGbpsPerService.getString(), ", "));
			...
			if (dM){
				...
				System.out.println("Simulation Mode:" + this._trafficType.getString ());
				System.out.println("Load Factor:" + this.fg_loadFactor.getDouble());
			...
			if (!cac_auxIncremental) holdingTimeGenerator_ds = new LinkedHashMap<Demand, AbstractDoubleDistribution[]>();	
			...
			{			
				...
				if (!this.cac_auxIncremental) holdingTimeGenerator_ds.put(originalDemand, new AbstractDoubleDistribution[numServices]);		
				
				final String[] aux_connectionProportionPerService = StringUtils.split(this.fg_connectionProportionPerService.getString(), ", ");
				...
				{
					final double iat_thisDemand_thisService = bandwidthInGbpsPerServiceArray[serviceId] / (this.fg_loadFactor.getDouble() * h_d * connectionProportionPerServiceArray[serviceId]);
					...
					if (!cac_auxIncremental) holdingTimeGenerator_ds.get(originalDemand)[serviceId] = new Exponential(1, new MersenneTwister64(this.rng.nextInt()));
					final double nextArrivalTime = 0 + interarrivalTimeGenerator_ds.get(originalDemand)[serviceId].nextDouble();
					double nextDuration = cac_auxIncremental ? Double.MAX_VALUE : holdingTimeGenerator_ds.get(originalDemand)[serviceId].nextDouble();
					scheduleEvent(new SimEvent(nextArrivalTime, SimEvent.DestinationModule.EVENT_PROCESSOR, -1, fg_loadFactor.getDouble()));					
					...
					scheduleEvent(new SimEvent(nextArrivalTime, SimEvent.DestinationModule.EVENT_GENERATOR ,-1, new GenerateConnectionRequestFlexgrid(originalDemand,nextDuration,serviceId)));
	...
	public void processEvent(NetPlan currentNetPlan, SimEvent event)
	{
		...
		if (eventObject instanceof GenerateConnectionRequestFlexgrid)
		{
			
			numberOfConnectionsRequests++;
			if (numberOfConnectionsRequests > fg_maxNumberOfConnectionRequests.getLong() && fg_maxNumberOfConnectionRequests.getLong() != -1){
				endSimulation();
			}
			
			GenerateConnectionRequestFlexgrid connectionRequest = (GenerateConnectionRequestFlexgrid) eventObject;
			...
			if (!cac_auxIncremental) scheduleEvent(new SimEvent(simTime + duration, SimEvent.DestinationModule.EVENT_GENERATOR , -1 , new GenerateConnectionRelease(routeInfo_add)));
			if (!cac_auxIncremental) scheduleEvent(new SimEvent(simTime + duration,SimEvent.DestinationModule.EVENT_GENERATOR,-1 ,routeInfo_add));

			final double nextArrivalTime = simTime + interarrivalTimeGenerator_ds.get(originalDemand)[serviceId].nextDouble();
			final double nextDuration = cac_auxIncremental ? Double.MAX_VALUE : holdingTimeGenerator_ds.get(originalDemand)[serviceId].nextDouble();
			scheduleEvent(new SimEvent(nextArrivalTime, SimEvent.DestinationModule.EVENT_GENERATOR , -1 , new GenerateConnectionRequestFlexgrid(originalDemand,nextDuration,serviceId)));
...
//			scheduleEvent(new SimEvent (simTime, SimEvent.DestinationModule.EVENT_PROCESSOR , -1 , demandOfferedTrafficUpdate));
	...
	private static class GenerateConnectionRequestFlexgrid
	{
		public final Demand demand;
		public final double duration;
		public final int serviceId;
		
		public GenerateConnectionRequestFlexgrid(Demand demand, double duration , int serviceId) {
			this.demand = demand; 
