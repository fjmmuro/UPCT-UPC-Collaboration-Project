 ...
 */
public class OnLine_EP_flexGrid_UPCT_UPC extends IEventProcessor
{
...
//		double[] u_e = initialNetPlan.getLinkCapacityVector();
		...
		if(this.timeTraceMode.getBoolean())
			statsCalculator.setStatisticsCalculator(initialNetPlan,trafficLayer,numSlotsPerService,totalAvailableSlotsPerFiber);
			...
			for (List<Link> seqFibers : paths)
			{
				...
				for (Pair<Integer, Integer> candidateVoid : candidateVoids)
				{
					...
					for (int initialSlotId = firstPossibleInitialSlotId; initialSlotId <= lastPossibleInitialSlotId; initialSlotId++)
					{
						...
						double final_metrics = metricRatioArray[0]*f_cmt/norm_c_cmt+metricRatioArray[1]/currentAlgConn;
